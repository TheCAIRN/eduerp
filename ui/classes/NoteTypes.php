...
/*
 !!!!!!!!!!!!! INCOMPLETE !!!!!!!!!!!!!!!!!!
*/
class NoteTypes extends ERPBase {
	...
	public function NoteTypesSelect($id=0,$readonly=false) {
		return parent::abstractSelect($id,$readonly,'aa_note_types','note_type_id','note_type_code','Note Type');
	} // AttachmentTypesSelect()
	public function listRecords() {
		parent::abstractListRecords('NoteTypes');
	} // listRecords()
	public function searchPage() {
		parent::abstractSearchPage('NoteTypesSearch');
	} // searchPage()
	public function executeSearch($criteria) {
		$q = "SELECT * FROM aa_note_types ";
		// TODO: Add $criteria
		// TODO: Convert to prepared statements
		$q .= " ORDER BY note_type_id";
		$result = $this->dbconn->query($q);
		if ($result!==false) {
			$this->recordSet = array();
			while ($row=$result->fetch_assoc()) {
				$this->recordSet[$row['note_type_id']] = array();
			} // while rows
		} // if query succeeded
		$this->listRecords();
		...
		$_SESSION['searchResults']['NoteTypes'] = array_keys($this->recordSet);		
	} // executeSearch()
